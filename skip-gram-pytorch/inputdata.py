...
class Options(object):
  def __init__(self, datafile, embeddingsfile, embedding_dim):
    ...
    self.embeddings = self.read_embeddings(embeddingsfile, self.vocab_words, embedding_dim)
  ...
  def read_embeddings(self, embeddingsfile, vocab_words, embedding_dim):
    emb = np.random.uniform(0.0, 1.0, (len(vocab_words), embedding_dim))
    with open(embeddingsfile, "r") as f:
      for line in f:
        vals = line.strip().split()
        if vals[0] in vocab_words:
          emb[vocab_words[vals[0]]] = map(lambda x: float(x), vals[1:])
    return emb
