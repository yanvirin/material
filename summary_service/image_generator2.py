...
COLOR_YELLOW = ImageColor.getrgb("#FFFF99")
COLOR_YELLOW = ImageColor.getrgb("#FFFFCC")
...
def wrap_text(text, max_width, font):
    ...
    while len(line):
        ...
        finished_lines.append(" ".join(current_buffer))
...
def wrap_line(line, max_width, font):
    ...
    while len(line):
        first_tok = line.pop(0)
        width = font.getsize(first_tok["word"])[0]
        current_buffer = [first_tok]
        
        while len(line):
            if line[0]["nl"]:
                break
            new_width = width + space_size + font.getsize(line[0]["word"])[0]
            if new_width < max_width:
                width = new_width
                current_buffer.append(line.pop(0))
            else:
...
def draw_line(line, draw, font, bold_font, y_pos):
    ...
    for t, token in enumerate(line):
        ...
        fill = token.get("color", COLOR_WHITE)
        if token["highlight"]:
            
            draw.rectangle((x_pos, y_pos, x_pos + w, y_pos + h), 
                           fill=COLOR_YELLOW)
            draw.text((x_pos, y_pos), token["word"], font=bold_font, 
                      fill=fill)
        else:
            draw.text((x_pos, y_pos), token["word"], font=font, fill=fill)
...
class SummaryImage(object):
    def __init__(self, width=1024, height=768):
        self.image = Image.new('RGBA', (width, height), COLOR_BLACK)
        self.width = width
        self._y_cursor = 15


    def append_message(self, message_text, message_color, font_size=25):
        font = ImageFont.truetype(
            '/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf', font_size)
        font_height = font.getsize(message_text)[1] + 5
        draw = ImageDraw.Draw(self.image)

        for line in wrap_text(message_text, self.width - 50, font):

            draw.text((25, self._y_cursor), line, font=font, 
                      fill=ImageColor.getrgb(message_color))
            self._y_cursor += font_height + 5

    def append_extract(self, extract, font_size=25):
        text = " ".join([tok["word"] for tok in extract["tokens"]])
        font = ImageFont.truetype(
            '/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf', font_size)
        bold_font = ImageFont.truetype(
        ...
        wrapped_tokens = wrap_line(extract["tokens"], self.width - 25, font)
        for wrapped_line in wrapped_tokens:
            draw_line(wrapped_line, draw, font, bold_font, self._y_cursor)
            self._y_cursor += 10 + font_height 
