...
def get_data(files, expect_labels=True, tokenize=False, verbose=False, files_already_opened=False):
    ...
    for file in files:
        ...
        for line in fh:
            ...
            ## make sure every line will be a sentence
    ...
    def segment(self, use_preds=False, tokenize=False, output=None, list_only=False):
        ...
        while frag:
            ...
            if frag.ends_seg or (use_preds and frag.pred>thresh) or (not use_preds and frag.label>thresh):
                ...
                sent_text = word_tokenize.normalize(' '.join(sent)) if tokenize else ' '.join(sent)
    ...
    def run_on(run_path):
      for f in os.listdir(run_path):
        ...
        if os.path.isfile(fp) and fp.endswith(".txt"):
          out_f = open(fp + ".nrm", "w")
          test = get_data(fp, tokenize=True)
          test.featurize(model, verbose=False)
          model.classify(test, verbose=False)
          test.segment(use_preds=True, tokenize=options.tokenize, output=out_f)
          out_f.close()
        if os.path.isdir(fp): run_on(fp)
  
    # assuming we got a dataset dir (Yan 2017-10-19)
