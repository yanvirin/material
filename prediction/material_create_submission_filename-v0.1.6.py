...
# -*-coding:Utf-8 -*
'''
    File name: material_create_submission_filename-v0.1.6.py
...
def renameSub():
	...
	parser.add_argument("--set", help="Choose a queryset (QUERY1: 300, QUERY2: 400, QUERY1QUERY2: 700, QUERY2QUERY3: 1000) OR a domainset (DX: GOV-LIF [1A/1B] GOV-MIL [1S], DXY: GOV-LIF-BUS-LAW [1A] GOV-LIF-HEA-MIL [1B] GOV-MIL-BUS-LAW [1S], DXYZ: GOV-LIF-BUS-LAW-SPO [1A] GOV-LIF-HEA-MIL-SPO [1B])", choices=['QUERY1','QUERY2','QUERY1QUERY2','QUERY2QUERY3','DX','DXY','DXYZ'])
	parser.add_argument("--lang", required=True, help="Choose a language", choices=['1A','1B','1S'])
	parser.add_argument("--dataset", required=True, help="Choose a dataset", choices=['DEV','ANALYSIS1','DEVANALYSIS1','EVAL1','EVAL1EVAL2','EVAL1EVAL2EVAL3','ANALYSIS1ANALYSIS2'])
	...
	if (args.task=="CLIR" or args.task=="E2E" or args.task=="DomainID") and args.set is None:
	...
	elif (args.task=="DomainID" and args.lang=="1S" and args.set=="DXY"):
		args.set="GOV-MIL-BUS-LAW"
